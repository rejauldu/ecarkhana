						...
						<table id="dataTables" class="display nowrap" cellspacing="0" width="100%">
							...
							<tbody>
								...
								<tr>
									<td>{{ $district->id }}</td>
									<td>{{ $district->name }}</td>
									<td><a href="{{ route('districts.edit', $district->id) }}" class="text-success fa fa-edit"></a></td>
									<td><a href="{{ route('districts.destroy', $district->id) }}" onclick="event.preventDefault(); document.getElementById('delete-form').action = this.href; document.getElementById('delete-form').submit.click();" class="text-danger fa fa-trash"></button></td>
								</tr>
								@endforeach
							</tbody>
							<tfoot>
